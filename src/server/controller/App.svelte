    ...
    switch (msg.channel) {
      case "OUTPUT_FRAME":
        ...
        if (msg.data.width && msg.data.height)
          aspectRatio = msg.data.width / msg.data.height
        frameReceived = true
        break
      case "GET_OUTPUT_ID":
        outputId = msg.data
...
{#if draw}
  <div class="draw">
    ...
    <div
      bind:this={padElem}
      class="pad"
      style="aspect-ratio: {thumbnailBackground ? aspectRatio : 1};"
      ...
      {#if thumbnailBackground}
        <div class="thumbnail">
          ...
          <img
            ...
            alt=""
            style="width: 100%;height: 100%;object-fit: fill;opacity: 0.5;"
          />
        </div>
      {/if}
    </div>
    ...
    {#if tool === "Paint"}
      <button
        on:click={() => sendAction("clear_painting")}
...
{:else}
  ...
  <button class="quart" /> -->
    <button
      class="center"
      class:red={justCleared}
      on:click={() => sendAction("clear")}
    >
...
<div class="toggles">
  <button
    on:click={() => (draw = false)}
    class="noright"
    style={draw ? "background-color: var(--primary-darker);" : ""}
  >
