...
<<<<<<< HEAD
  ...
  function extractLinksAndCleanText(text: string) {
    ...
    const textWithoutATags = text.replace(
      /<a\s[^>]*href=["'](https?:\/\/[^"']+)["'][^>]*>.*?<\/a>/gi,
      (_match, url) => {
        links.push(url)
        return ""
    ...
    const finalText = textWithoutATags.replace(
      /https?:\/\/[^\s<>"']+/gi,
      (url) => {
        links.push(url)
        return ""
...
<Center class="context #splash">
  <h1 style="font-size: 3em;">RCF VisionCast</h1>
    ...
    <Button
      on:click={(e) => {
        if (e.ctrlKey || e.metaKey) {
          history({
            id: "UPDATE",
            newData: { remember: { project: $activeProject } },
